...
function highlightSyntax(code: string): string {
  return code
    .replace(/(\/\/.*$)/gm, '<span class="text-muted-foreground/80">$1</span>')
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.04]">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-3 flex items-center gap-1.5">
            ...
            <span className="text-xs font-mono text-muted-foreground">{snippet.title}</span>
          </div>
        </div>
      ...
      <div className="relative min-h-[250px]">
        ...
        {/* Line numbers gutter */}
        <div className="absolute top-5 left-0 flex flex-col font-mono text-sm leading-relaxed text-muted-foreground/80 pl-2 select-none pointer-events-none">
          {snippet.code.split('\n').map((_, i) => (
            <span key={i}>{i + 1}</span>
          ))}
        </div>
