    ...
    <div className="relative min-h-screen pt-24 pb-24 px-4 overflow-hidden">
      ...
      <div className="container mx-auto max-w-6xl relative">
        ...
        >
          ...
          <p className="text-muted-foreground/50 mb-8">Professional credentials & achievements</p>
        ...
        >
          <div className="premium-card p-4 !rounded-xl text-center">
            ...
            <div className="text-xs text-muted-foreground/50">Certifications</div>
          </div>
          <div className="premium-card p-4 !rounded-xl text-center">
            <div className="text-2xl md:text-3xl font-bold gradient-text-premium">{issuers.length}</div>
            <div className="text-xs text-muted-foreground/50">Issuers</div>
          </div>
          <div className="premium-card p-4 !rounded-xl text-center">
            <div className="text-2xl md:text-3xl font-bold gradient-text-premium">{yearsLearning}+</div>
            <div className="text-xs text-muted-foreground/50">Years Learning</div>
          </div>
        </motion.div>
                ...
                >
                  <Wrapper {...(wrapperProps as any)} className={`block ${isClickable ? 'cursor-pointer' : ''}`}>
                    <div className={`premium-card p-6 h-full transition-all group ${
                      ...
                      <p className="text-sm text-muted-foreground/50 mb-3">{cert.issuer}</p>
                      ...
                      {Array.isArray(cert.tags) && cert.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-4">
                          {cert.tags.slice(0, 3).map((t: string) => (
                            <span key={t} className="px-2 py-0.5 rounded-md bg-white/[0.03] text-[10px] text-muted-foreground/50">
                              {t}
                            </span>
                          ))}
                        </div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground/50">No certifications found</p>
          </div>
        )}
      </div>
    </div>
