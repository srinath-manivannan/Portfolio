          ...
          >
            {isSuccess ? (
              ...
              >
                ...
                <p className="text-muted-foreground/50 mb-6 text-sm">Thanks for reaching out. I'll get back to you within 24 hours.</p>
                <Button variant="outline" onClick={() => setIsSuccess(false)} className="rounded-xl border-white/[0.06]">Send Another</Button>
              </motion.div>
            ) : (
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-xs font-semibold mb-1.5 text-foreground/70 tracking-wide uppercase">Name *</label>
                  <Input
                    type="text"
                    value={formData.name}
                ...
                <div>
                  <label className="block text-xs font-semibold mb-1.5 text-foreground/70 tracking-wide uppercase">Email *</label>
                  <Input
                    type="email"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5 text-foreground/70 tracking-wide uppercase">Subject</label>
                    <Input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-semibold mb-1.5 text-foreground/70 tracking-wide uppercase">
                      Category
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <label className="text-xs font-semibold text-foreground/70 tracking-wide uppercase">Message *</label>
                    <span className={`text-[10px] ${charCount > 1800 ? 'text-destructive' : 'text-muted-foreground/30'}`}>
                      {charCount}/2000
                    </span>
                  </div>
                  <Textarea
                ...
                <div key={item.label} className="premium-card p-5 group">
                  {item.href ? (
                    <a href={item.href} target={item.href.startsWith('mailto') ? undefined : '_blank'} rel="noopener noreferrer" className="block">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-[10px] text-muted-foreground/30 font-medium tracking-wider uppercase">{item.label}</p>
                          <p className={`text-sm font-medium truncate group-hover:text-primary transition-colors ${item.isStatus ? 'text-green-400/80' : 'text-foreground/70'}`}>
                            {item.value}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] text-muted-foreground/30 font-medium tracking-wider uppercase">{item.label}</p>
                        <p className={`text-sm font-medium truncate ${item.isStatus ? 'text-green-400/80' : 'text-foreground/70'}`}>{item.value}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="premium-card p-6">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-sm text-foreground/70">WhatsApp</p>
                ...
                >
                  ...
                  <div>
                    <p className="font-medium text-sm text-foreground/70">LinkedIn</p>
